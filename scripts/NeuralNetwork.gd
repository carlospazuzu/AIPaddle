...
func _train(inputs, y_true):
	...
	var sum_h1 = weights[0] * inputs[0] + weights[1] * inputs[1] + biases[0]
	var h1 = _sigmoid(sum_h1)
	
	var sum_h2 = weights[2] * inputs[0] + weights[3] * inputs[1] + biases[1]
	var h2 = _sigmoid(sum_h2)
	
	var sum_o1 = weights[4] * h1 + weights[5] * h2 + biases[2]
	...
	var d_ypred_d_h1 = weights[4] * _deriv_sigmoid(sum_o1)
	...
	# neuron o1
	weights[4] -= learning_rate * d_L_d_y_pred * d_ypred_d_w5
	weights[5] -= learning_rate * d_L_d_y_pred * d_ypred_d_w6
	biases[2] -= learning_rate * d_L_d_y_pred * d_ypred_d_b3
